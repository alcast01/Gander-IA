...
st.markdown("""
    <div style="display: flex; align-items: center; background: linear-gradient(135deg, #2d5a27 0%, #bc6c25 100%); padding: 15px; border-radius: 12px; box-shadow: 0 4px 12px rgba(45,90,39,0.15); margin-bottom: 15px; color: white; flex-wrap: wrap; gap: 10px;">
        ...
        <div style="flex-grow: 1; min-width: 200px;">
            ...
            <p style="margin: 2px 0 0 0; font-size: 0.85em; color: #f4f1de; font-weight: 300; font-family: 'Inter', sans-serif;">Optimización, Panel Compacto y Sostenibilidad</p>
        </div>
    </div>
""", unsafe_allow_html=True)
...
with st.sidebar.expander("🐄 1. Lote y Población", expanded=True):
    cantidad_animales = st.number_input("Número de Cabezas", min_value=1, max_value=5000, value=100, step=10)
    ...
    gde = st.slider("Ganancia Esperada (kg/día)", min_value=0.8, max_value=2.2, value=1.4, step=0.1)

with st.sidebar.expander("💰 2. Parámetros Económicos", expanded=False):
...
with st.sidebar.expander("🌾 3. Sistema y Pastoreo", expanded=False):
...
with st.sidebar.expander("🧬 4. Genética y Categoría", expanded=False):
